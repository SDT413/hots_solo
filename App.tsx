...
import { RankId, Rank, Character, ExportData } from './types';
...
function App() {
  ...
  const [gridState, setGridState] = useState<RankId[][]>(() =>
    Array(characters.length).fill(null).map(() => Array(characters.length).fill(DEFAULT_RANK_ID))
  );
  ...
  const handleCellClick = useCallback((rowIndex: number, colIndex: number) => {
    ...
    setGridState(prevState => {
      ...
      newState[rowIndex][colIndex] = selectedRank;
      return newState;
    });
  }, [selectedRank]);
  ...
  const handleReset = useCallback(() => {
     setGridState(Array(characters.length).fill(null).map(() => Array(characters.length).fill(DEFAULT_RANK_ID)));
  }, [characters]);
  ...
  const handleDeleteRank = useCallback((rankIdToDelete: RankId) => {
    setGridState(prevState => 
      prevState.map(row => 
        row.map(cellRankId => 
          cellRankId === rankIdToDelete ? DEFAULT_RANK_ID : cellRankId
        )
      )
    );
  ...
  const handleImport = useCallback((data: ExportData) => {
    if (!data.characters || !data.ranks || !data.gridState || 
        !Array.isArray(data.characters) || !Array.isArray(data.ranks) || !Array.isArray(data.gridState)) {
      ...
      <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col items-center p-4 sm:p-8 font-sans">
        <header className="w-full max-w-7xl text-center mb-8">
          ...
          <p className="text-lg text-gray-400 mt-2">
            Who did the opponent pick for the solo lane? Select a rank and build your counter-pick chart.
          </p>
        </header>

        <main className="w-full max-w-[1600px] flex flex-col gap-8 items-center justify-center">
